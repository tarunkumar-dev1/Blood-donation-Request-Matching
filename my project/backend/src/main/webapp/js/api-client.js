(function(){
  ...
  const defaultBase = isFile ? 'https://blood-donation-request-matching.onrender.com/api' : '/api';
  ...
  document.addEventListener('DOMContentLoaded', function(){
    ...
    bind('form[data-api="match"]', async (data, form) => {
      ...
      if (resp.success) {
        ...
        alert('Request submitted successfully!\n\nMatches found: ' + (resp.adminMatchesCreated || resp.matches?.length || 0));
        form.reset();
      } else {
        throw new Error(resp.error || 'Failed to submit request');
      }
    });
  });
